  ...
  if (session.user_role !== "regular") {
    return {
      ...
      response: NextResponse.json(
        {
          ok: false,
          message: "Endpoint profile ini hanya untuk user regular",
        },
        {
          status: 403,
        },
  ...
  try {
    ...
    if (!profile) {
      return NextResponse.json(
        {
          ok: false,
          message: "Profile regular tidak ditemukan",
        },
        {
          status: 404,
        },
    ...
    const message =
      error instanceof Error ? error.message : "Gagal mengambil profile regular";
...
export async function PUT(request: Request) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Gagal memperbarui profile regular";
