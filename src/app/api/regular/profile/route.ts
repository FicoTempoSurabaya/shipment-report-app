  ...
  if (session.user_role !== "regular") {
    return {
      ...
      response: NextResponse.json(
        { ok: false, message: "Endpoint profile ini hanya untuk user regular" },
        { status: 403 },
      ),
    };
  }
...
export async function GET() {
  try {
    ...
    const rows = await query<RegularProfileRow>`
      SELECT
    ...
    if (!profile) {
      return NextResponse.json(
        { ok: false, message: "Profile regular tidak ditemukan" },
        { status: 404 },
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Gagal mengambil profile regular";
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        {
          ok: false,
          message: "Input profile tidak valid",
    ...
    return NextResponse.json({
      ok: true,
      message: "Profile regular berhasil diperbarui",
      data: updatedProfile,
    });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Gagal memperbarui profile regular";
